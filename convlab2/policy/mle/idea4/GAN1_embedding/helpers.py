...
def prepare_discriminator_data(pos_samples, neg_samples, ratio_pos, ratio_neg, gpu=False):
    """
    :param pos_samples:[1, num_samples, 549] or [1, num_samples, 1024]
    :param neg_samples:[1, num_samples, 549] or [1, num_samples, 1024]
    :param gpu: True
    ...
    if gpu:
        inp = inp.cuda()
        target = target.cuda()
    # Todo: what about extra dataset. Ignore them, you have to keep the number same.
    return inp, target
