...
from train_dialogue import loss_fn

class Compare():
    def __init__(self):
        pass

    def cosine(self, input, oracle):
...
class dialogue_VAE(nn.Module):
    def __init__(self, embedding_size, rnn_type, hidden_size, word_dropout, embedding_dropout, latent_size,
        ...
        self.linear_output1 = nn.Linear(hidden_size * self.hidden_factor + hidden_size,hidden_size)
        self.linear_output2 = nn.Linear(hidden_size, 9)
    ...
    def forward(self, input_sequence, mask_id, input_belief):
        ...
        whole_pack, hidden = self.encoder_rnn(packed_input)

        # domain classifier
        padded_outputs_whole = rnn_utils.pad_packed_sequence(whole_pack, batch_first=True,padding_value=0.0)[0]
        padded_outputs_whole = padded_outputs_whole.contiguous()
        _, reversed_idx = torch.sort(sorted_idx)
        padded_outputs_whole = padded_outputs_whole[reversed_idx]

        input_hidden = torch.tensor(data=[])
        for i in range(len(mask_id)):
            mask = mask_id[i]-1
            one = padded_outputs_whole[i][mask].unsqueeze(0)
            input_hidden = torch.cat((input_hidden, one.cpu()), 0)
        input_hidden = input_hidden.to(self.device).view(batch_size, self.hidden_size * self.hidden_factor)
        input_belief = torch.stack(input_belief).float().to(self.device)
        input_belief = self.linear_belief(input_belief)
        domain_predict = torch.cat((input_hidden,input_belief),dim=1)
        ...
        return original_input_tensor, reconstructed_dialogue, domain_predict
        ...
        for i in range(len(batch)):
            ...
            for j in range(len(temp)):
                ...
                if len(input) > 10:
                    ...
                    loss1, loss2 = loss_fn(logp, original_input.to(self.device), disc_res,
                                           torch.tensor(discriminator_target).float().to(self.device))
                    loss = loss1 + loss2 * 2
    ...
    def get_score_domain(self, input):
        ...
        for i in range(len_dial):
            input_hidden = torch.zeors(1, self.hidden_size * self.hidden_factor)
            if i != 0:
            ...
            input_belief = self.linear_belief(input_belief[i].unsqueeze(0).to(self.device))
            domain_predict = torch.cat((input_hidden, input_belief), dim=1)
            domain_predict = self.linear_output2(F.relu(self.linear_output1(domain_predict)))

            score = 0.5 - self.bceLoss(domain_predict, torch.tensor(domain_list[i]).to(self.device))
