...
root_dir = "/dockerdata/siyao/ft_local/ConvLab/"
...
class PPO(Policy):
    def __init__(self, args, seed, is_train=False, dataset='Multiwoz'):
        ...
        if is_train:
            self.policy_optim = optim.Adam(self.policy.parameters(), lr=cfg['policy_lr'])
            self.value_optim = optim.Adam(self.value.parameters(), lr=cfg['value_lr'])

        self.reward_predictor1 = Model1(549, "gru", 512, 1, 1, 256, 60, 1, True).to(DEVICE)
        self.reward_predictor2 = Model2().to(DEVICE)
        self.seed = seed
    ...
    def reward_estimate(self, r, s, a, mask):
        ...
        """
        ae_reward = self.reward_predictor1.get_reward(r.clone().detach(), s.clone().detach(), a.clone().detach(), mask.clone().detach(), self.args.update_ae)
        domain_reward = self.reward_predictor2.get_reward(r.clone().detach(), s.clone().detach(), a.clone().detach(), mask.clone().detach(), self.args.update_domain)
        reward_predict = self.args.ae_weight * ae_reward.to(DEVICE) + self.args.domain_weight * domain_reward.to(DEVICE) + self.args.r_weight * r + self.args.bias * mask
        print(reward_predict[:15])
    ...
    def update(self, epoch, batchsz, s, a, r, mask):
        ...
        r = self.reward_estimate(r,s,a,mask).detach()
        ...
        if (epoch + 1) % self.save_per_epoch == 0:
            self.save(self.save_dir+str(self.args.ae_weight)+str(self.args.domain_weight)+str(self.args.bias)+"seed"+str(self.seed), epoch)
    ...
    def load_reward_model1(self, filename):
        policy_mdl_candidates = [
            filename,
            filename + '.pol.mdl',
            ...
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '.pytorch')
        ]
        for policy_mdl in policy_mdl_candidates:
            if os.path.exists(policy_mdl):
                self.reward_predictor1.load_state_dict(torch.load(policy_mdl, map_location=DEVICE))
                logging.info('<<dialog policy>> loaded reward model checkpoint from file: {}'.format(policy_mdl))
                break

    def load_reward_model2(self, filename):
        policy_mdl_candidates = [
            filename,
            filename + '.pol.mdl',
            filename + '_pg.pol.mdl',
        ...
        for policy_mdl in policy_mdl_candidates:
            if os.path.exists(policy_mdl):
                self.reward_predictor2.load_state_dict(torch.load(policy_mdl, map_location=DEVICE))
