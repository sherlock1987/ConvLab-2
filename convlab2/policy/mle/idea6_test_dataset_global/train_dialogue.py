...
def main(args):
        ...
        for split in splits:
            ...
            for iteration, batch in enumerate(data_loader_real):
                if batch.size(1) >1:
                    ...
                    # 实际上我训练了一个比较奇怪的模型，我想想看
                    if (iteration+1) % (args.batch_size) == 0:
                        batch_size = len(temp)
                        # Forward path for VAE
                        original_input, logp = model(temp)
                        # original_input, logp, mean, logv, z = model(temp, max_len)

                        # loss calculation
                        disc_res = []
                        loss1 = loss_fn(logp, original_input.to("cuda"), disc_res, torch.tensor(discriminator_target).float().to("cuda"))
                        loss = loss1
                        if (batch_id+1) % 1000 == 0:
                            print("loss1 & 2:",loss1.item()/batch_size)
    ...
    save_path = "./bin/"
    torch.save(model.state_dict(), save_path + "idea6_domain_tiny.pol.mdl")
...
if __name__ == '__main__':
    ...
    from convlab2.policy.mle.idea6_test_dataset_global.model_dialogue import dialogue_VAE
    main(args_idea5)
