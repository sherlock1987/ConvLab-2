...
class Generator(nn.Module):
    ...
    def sample(self, input_samples):
        ...
        4. out put.
        """
        # Todo, no gradient, only using it. No gradient.
        # set clip
        samples_pos = tensor([])
        samples_neg = tensor([])
        #
        ...
        for i, ele in enumerate(input_samples):
            ...
            fake_action = self.predict_act(prev, bf)
            last_fake_cur = torch.cat((bf, fake_action[0][0].unsqueeze(0).unsqueeze(0)), 2)
        ...
        # test similarity
        # This stuff is almost same, so we should change to 123 model and try to debug.
        output = torch.cosine_similarity(samples_pos, samples_neg, dim=2)
