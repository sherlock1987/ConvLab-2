...
def main(args):
    ...
    for epoch in range(args.epochs):
        for split in splits:
            ...
            mask_id_collc = []
            mask_list = []
            # you should use zip to do this stuff.
            for i, batch in enumerate(data_loader_real):
                one_1, one_2, one_3=  data_mask(batch)
                data_collc += one_1
                mask_id_collc += one_2
                domain_collc += one_3

            assert len(data_collc) == len(domain_collc)
            index = [i for i in range(len(domain_collc))]
            for batch, domain, mask, iteration in zip(data_collc, domain_collc, mask_id_collc, index):
                if batch.size(1) >1:
                    temp.append(batch)
                    ...
                    if (iteration+1) % (args.batch_size) == 0:
                        ...
                        original_input, logp, disc_res = model(temp, mask_list)
