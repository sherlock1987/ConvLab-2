...
import os
from convlab2.policy.mle.idea4.GAN1_embedding.generator import Generator
from convlab2.policy.mle.idea4.GAN1_embedding.discriminator import Discriminator
import convlab2.policy.mle.idea4.GAN1_embedding.helpers
from convlab2.policy.mle.idea4.GAN1_embedding.helpers import prepare_data, prepare_discriminator_data, oracle_sample, batchwise_sample, batchwise_oracle_nll
import pickle
import random
...
load_VAE_path = os.path.join(root_dir, "mle/idea4/model/VAE_1_123.pol.mdl")
# path stuff
pretrained_gen_path = os.path.join(root_dir, "mle/idea4/GAN1_embedding/Gen")
pretrained_dis_path = os.path.join(root_dir, "mle/idea4/GAN1_embedding/Dis")
...
def train_generator_MLE(gen, gen_opt, real_data_samples, epochs = 10):
    ...
    for epoch in range(epochs):
        ...
        for iteration, ele in enumerate(real_data_samples[seq_train[0]]):
            ...
            if ceil((iteration / BATCH_SIZE_G) % 100) == 0:
                print('.', end='')
                sys.stdout.flush()
...
def train_discriminator(discriminator, dis_opt, real_data_samples, generator, sample_num, ratio_pos = 1.0 ,ratio_neg = 1.0, d_steps = 1, epochs = 30):
    ...
    for d_step in range(d_steps):
        ...
        train_inp, train_target = prepare_discriminator_data(pos_train, neg_trian, ratio_pos, ratio_neg, gpu=CUDA)
        ...
        for epoch in range(epochs):
            ...
            # go to the batch
            batch_num = np.ceil((train_inp.size(1)) / BATCH_SIZE_D)
            ...
            val_inp, val_target = prepare_discriminator_data(pos_val, neg_val, ratio_pos, ratio_neg, gpu=CUDA)
            # do loss again
...
if __name__ == '__main__':
    ...
    print("Data [prev: bf: a] Loading Successful")
    ...
    train_discriminator(dis, dis_optimizer, datasets, gen, sample_num=5000, ratio_pos = 1.0, ratio_neg = 1.0, d_steps= 3, epochs=20)
    torch.save(dis.state_dict(), os.path.join(pretrained_dis_path, "pretrain_D.mdl".format(ts)))
